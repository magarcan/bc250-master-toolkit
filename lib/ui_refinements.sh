...
# Canonical UI status semantics. All interactive status markers use these helpers.
...
ui_cpu_load_percent() {
  ...
  dt=$((t2-t1)); di=$((i2-i1))
  if [ "$dt" -gt 0 ]; then printf '%d' $((100*(dt-di)/dt)); else printf '0'; fi
}
...
ui_preflight() {
  ...
  local missing=() cores threads
  heading 'Platform'
  bc250_bios_ok && ok "BIOS $(bc250_bios) detected" || { warn "BIOS $(bc250_bios) — P3.00 recommended"; missing+=("BIOS P3.00"); }
  ...
  if bc250_bios_ok && bc250_kernel_ok; then ok 'ACPI: native BC-250 support — no ACPI override required'; else warn 'ACPI: cannot validate native support until BIOS and kernel are correct'; fi
  ...
  if [ "${#missing[@]}" -eq 0 ]; then
    printf '\n'; ok 'Platform validation: PASS'; printf '        BIOS, kernel, CPU topology, ACPI and toolkit dependencies are ready.\n'; info 'No ACPI override or software core-unlock action is required.'; return 0
  fi
  printf '\n'; warn "Platform validation: INCOMPLETE (${#missing[@]} item(s) require attention)"
