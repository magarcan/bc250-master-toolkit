...
# The governor TOML is root-owned by design. Status commands consume a small,
# non-sensitive public state file instead of requiring access to that TOML.
...
bc250_gpu_oc_status() {
  ...
  min=${range%%|*}; max=${range#*|}
  [ -n "$min" ] && [ -n "$max" ] || { warn 'GPU OC state is not available; run a privileged OC operation once.'; return 1; }
