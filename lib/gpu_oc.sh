...
bc250_gpu_oc_profile_value() {
  ...
  [ -r "$file" ] || return 1
  awk -v n="$name" -v k="$key" '$0~/^name = /{cur=$0;gsub(/^name = |"/,"",cur)} cur==n && $0~("^" k "[[:space:]]*="){sub(/^[^=]*=[[:space:]]*/,"");gsub(/"/,"");print;exit}' "$file"
}
...
bc250_gpu_oc_write() {
  ...
  python3 - "$GPU_OC_CONFIG" "$tmp" "$m" "$f" "$v" <<'PY'
import re,sys
src,dst,mn,mx,v=sys.argv[1:]
...
bc250_gpu_oc_apply_values() {
  ...
  r=$(bc250_gpu_oc_range 2>/dev/null || true); [ -z "$m" ] && m=${r%%|*}; [ -z "$m" ] && m=300
