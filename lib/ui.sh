...
ui_status() {
  ...
  printf '\nLive snapshot only. Configuration checks and remediation are handled by Preflight and the relevant setup screens.\n'
}
...
ui_require_root() { [ "$EUID" -eq 0 ] && "$ROOT/bc250-master-toolkit" "$@" && return $?; command -v sudo >/dev/null 2>&1 || { die 'sudo is required for this operation.'; return 1; }; sudo -v || { die 'Authorization was cancelled.'; return 1; }; sudo env BC250_PRIVILEGED=1 "$ROOT/bc250-master-toolkit" "$@"; }
...
ui_extras_status() {
  ...
  if [ -n "$conf" ]; then
    grep -q 'loglevel=0' "$conf" 2>/dev/null && printf '  RDSEED warning hide    enabled\n' || printf '  RDSEED warning hide    not configured\n'
...
bc250_ui_main() {
  case "${1:-menu}" in
    menu) ui_menu;; preflight) ui_preflight;; status) ui_status;; gpu) case "${2:-status}" in status) ui_gpu_status;; oc) case "${3:-status}" in profiles) printf 'Stock 1850 MHz @ 930 mV\nBalanced 2000 MHz @ 1000 mV\nAggressive 2100 MHz @ 1025 mV\nMaximum Experimental 2200 MHz @ 1050 mV\n';; status) bc250_gpu_oc_status;; apply) bc250_gpu_oc_apply "${4:-}";; manual) bc250_gpu_oc_manual "${4:-}" "${5:-}" "${6:-}";; reset) bc250_gpu_oc_reset;; *) die 'Unknown GPU OC command.'; return 1;; esac;; *) ui_gpu_status;; esac;; cpu) ui_banner; printf 'CPU: %sC / %sT\nDriver: %s\nGovernor: %s\nFrequency: %s MHz\n' "$(bc250_cpu_cores)" "$(bc250_cpu_threads)" "$(bc250_cpu_driver)" "$(bc250_cpu_governor)" "$(bc250_cpu_freq)";; memory) ui_banner; bc250_memory_status; echo; bc250_memory_recommendations;; extras) case "${2:-status}" in status) bc250_extras_status;; swap) [ "${3:-}" = enable ] || die 'Use: extras swap enable [size]'; ui_require_root extras swap enable "${4:-16G}";; zswap) ui_require_root extras zswap enable;; rdseed) ui_require_root extras rdseed hide;; mitigations) [ "${3:-}" = off ] || die 'Use: extras mitigations off'; ui_require_root extras mitigations off;; *) die 'Unknown extras command.'; return 1;; esac;; cu) case "${2:-status}" in install) ui_require_root cu install;; umr) ui_require_root cu umr install;; status) bc250_cu_status;; *) die 'Unknown CU command.'; return 1;; esac;; help|-h|--help) printf 'CachyOS BC250 Master Toolkit v%s\n' "$VERSION";; *) die "Unknown command: $1"; return 1;; esac
}
