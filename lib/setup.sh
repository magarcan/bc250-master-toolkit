...
bc250_setup_require_root() {
  bc250_setup_is_root && return 0
  if ! command -v sudo >/dev/null 2>&1; then
    die 'sudo is required for this operation.'
  fi
  sudo -v || die 'Authorization was cancelled.'
  sudo "$ROOT/bc250-master-toolkit" __root "$@"
}

bc250_kernel_package_installed() {
  pacman -Q linux-cachyos-bc250 >/dev/null 2>&1
}

bc250_kernel_active_ok() {
  uname -r | grep -qi -- 'bc250'
}

bc250_kernel_repo_configured() {
  grep -qE '^\[bc250-cachyos\][[:space:]]*$' "$BC250_PACMAN_CONF" 2>/dev/null
}

bc250_kernel_repo_configure() {
  bc250_setup_is_root || return 1
  [ -f "$BC250_PACMAN_CONF" ] || die "$BC250_PACMAN_CONF not found."
  if bc250_kernel_repo_configured; then return 0; fi
...
bc250_kernel_install() {
  ...
  echo
  warn 'Reboot is required to start the newly installed kernel.'
  printf '  Active kernel now : %s\n' "$(uname -r)"
  printf '  Installed kernel  : linux-cachyos-bc250\n'
}

bc250_kernel_setup_menu_action() {
  banner
  heading 'Kernel Setup'
  echo
  if bc250_kernel_active_ok; then
    ok "Active kernel: $(uname -r)"
    echo
    info 'The active kernel already contains the BC-250 marker.'
    return 0
  fi
  warn "Active kernel: $(uname -r)"
  echo
  ...
  printf 'It includes the BC-250 kernel patches and matching headers.\n\n'
  printf 'Install it now? [Y/n]: '
  read -r answer
  case "$answer" in n|N) info 'Kernel installation skipped.'; return 0;; esac
  bc250_setup_require_root kernel-install
}

bc250_bios_setup_menu_action() {
  banner
  heading 'BIOS Recommendation'
  echo
  if bc250_bios_ok; then
    ok "BIOS P3.00 detected: $(bc250_bios)"
    echo
    info 'No BIOS action is required from the toolkit.'
    return 0
  fi
  warn "Detected BIOS: $(bc250_bios)"
  echo
  ...
  printf 'The toolkit will NOT flash your BIOS. You must perform the\n'
  printf 'firmware installation yourself using the project instructions.\n\n'
  printf 'Firmware project:\n  %s\n\n' "$BC250_BIOS_URL"
  printf 'Latest release:\n  %s\n\n' "$BC250_BIOS_RELEASE_URL"
  printf 'Open the firmware project in your browser? [Y/n]: '
  read -r answer
